...
const step2Schema = z.object({
    ...
    anno_produzione: z.number().int().min(1900).max(2030),
...
export default function NuovaPraticaPage() {
    ...
    const descrizione = form2.watch("descrizione_tecnica");
    const funzioneAttuale = form2.watch("funzione_principale");

    useEffect(() => {
        if (descrizione && descrizione.length > 15 && !funzioneAttuale) {
            const guess = guessFunzione(descrizione);
            if (guess) form2.setValue("funzione_principale", guess, { shouldValidate: false });
        }
    }, [descrizione, funzioneAttuale, form2]);

    const onStep1 = (data: Step1Data) => { setStep1Data(data); setStep(1); };
    ...
    const onStep2 = async (data: Step2Data) => {
        ...
        setError(null);
        try {
            const res = await fetch("/api/v1/pratiche", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...step1Data, macchinario: data }) });
    ...
    return (
        <div className="max-w-2xl mx-auto animate-fade-in">
            <div className="mb-8">
                <button onClick={() => step === 0 ? router.push("/pratiche") : setStep(0)} className="flex items-center gap-2 text-slate-400 hover:text-white text-sm mb-4 transition">
                    ...
                    {STEPS.map((s, i) => (
                        <div key={s} className="flex items-center gap-3">
                            <div className="flex items-center gap-2">
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition ${i < step ? "bg-blue-600 border-blue-600 text-white" : i === step ? "border-blue-500 text-blue-400" : "border-slate-600 text-slate-600"}`}>
                                    {i < step ? <CheckCircle className="w-4 h-4" /> : i + 1}
                                </div>
                                <span className={`text-sm font-medium ${i === step ? "text-white" : i < step ? "text-blue-400" : "text-slate-600"}`}>{s}</span>
                            </div>
            ...
            {step === 0 && (
                <form onSubmit={form1.handleSubmit(onStep1)} className="glass-card p-6 space-y-5">
                    ...
                    <FieldGroup label="Data Prevista Arrivo" error={form1.formState.errors.data_prevista_arrivo?.message}>
                        ...
                        <p className="text-xs text-slate-500 mt-1">Facoltativa — puoi inserirla in seguito</p>
                    </FieldGroup>
            ...
            {step === 1 && (
                <form onSubmit={form2.handleSubmit(onStep2)} className="glass-card p-6 space-y-5">
                    ...
                    <FieldGroup label="Funzione Principale *" error={form2.formState.errors.funzione_principale?.message}>
                        <select {...form2.register("funzione_principale")} className={inputClass}>
                    ...
                    <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg transition">
                        {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Creando...</> : <><CheckCircle className="w-4 h-4" /> Crea Pratica</>}
                    </button>
                </form>
            )}
        </div>
